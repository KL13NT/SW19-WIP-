...
class MainDisplay extends Component{
    ...
    writeText = (y)=>{ //Text write control
      let i = 0
      const writingInterval = setInterval(()=>{ //TEXT WRITING ANIMATION
        this.setState({
    ...
    flicker = (x, y)=>{ //Text flicker animation
      ...
      const flickeringInterval = setInterval(()=>{  //FLICKERING ANIMATION 
        if(x > 0){
          if(scoreAppearance){
            this.setState({
    ...
    removeText = (y)=>{
      ...
      const textRemovalInterval = setInterval(()=>{
